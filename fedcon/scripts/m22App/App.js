...
function validateM22() {
    ...
    if(valuesArrM22.includes(" ")!=true){
        ...
        globalJqueryM22.fadeInFadeOut(true, "#resourcesM22D");   
...
function showElectricalData(dataObject) {
    ...
    for(let i in dataObject){
        if(i==="milType"||i==="rpNumber") {
            ...
            $(displayElectricalId[counter++]).val(dataObject[i]);
        }
    }
}
function showElectricalPosition(electricalPosition) {
    $("#showElectricalPositionM22").val(utilsM22.displayElectricalPosition(electricalPosition));
}
function showShaftInfo(shaftMounting) {
    ...
    for(let i in dataObject){
        if(i==="symbol") {
            ...
            $(displayShaftID[counter++]).val(dataObject[i]);
        }
    }//BADNEW;-)       
}   
